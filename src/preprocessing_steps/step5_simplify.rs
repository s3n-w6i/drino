...
use crate::preprocessing_steps::step4_merge_data::DatasetMergeOutput;
use crate::routing::algorithm::PreprocessingInput;
...
pub async fn simplify(
    ...
    let stops = stops
        .select([
            // Keep "old" id-pais (stop_id + dataset_id) so that we can match in other tables
